...
score_correct_i = 1（absolute_error_i <= 0.15），否则为 0
...
当前代码阈值：判定准确率 `>= 0.90` 且 MAE `<= 0.15` 为 `passed`；判定准确率 `>= 0.75` 且 MAE `<= 0.25` 为 `needs_review`；否则为 `rejected`。单项分数误差不超过 `0.15` 计为判定正确，误差在 `(0.15, 0.25]` 内计为部分一致。阈值是评估规则，不是把结果强行抬高到目标值。

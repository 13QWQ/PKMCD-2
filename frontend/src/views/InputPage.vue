...
const selectedJob = computed(() => asArray<JobInfo>(jobs.value).find(job => job.id === selectedJobId.value) || null)
...
const accumulatedReviewText = computed(() => [...asArray<ReviewMessage>(reviewMessages.value).filter(message => message.role === 'user').map(message => message.content), userInput.value].join(' '))
...
const progressEvents = computed(() => asArray<AssessmentProgress['events'][number]>(liveProgress.value?.events).slice(-6).reverse())
...
async function loadJobs() {
  ...
  if (publicPreview || demoMode.value) {
    ...
    try { jobs.value = await getJobList(); if (!jobs.value.length) jobError.value = true } catch { jobs.value = []; jobError.value = true }
  }
...
async function restoreReviewSession() {
  ...
  try {
    ...
    reviewMessages.value = asArray<ReviewMessage>(messages)
    ...
    reviewMissing.value = asStringArray(session.review_state?.missing)
    reviewHint.value = typeof session.review_state?.last_question === 'string' ? session.review_state.last_question : ''
    store.setCurrentSession(session.id)
  } catch {
...
function recordDialogueTurn(content: string, result: Awaited<ReturnType<typeof submitReviewTurn>>) {
  ...
  reviewMissing.value = asStringArray(result.missing)

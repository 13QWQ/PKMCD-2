...
function normalizeAssessmentProgress(value: unknown): AssessmentProgress {
  const item = asRecord(value)
  return {
    stage: typeof item.stage === 'string' ? item.stage as AssessmentProgress['stage'] : 'material',
    agent: typeof item.agent === 'string' ? item.agent : '资料解析 Agent',
    label: typeof item.label === 'string' ? item.label : '等待开始',
    percent: typeof item.percent === 'number' && Number.isFinite(item.percent) ? item.percent : 0,
    status: typeof item.status === 'string' ? item.status as AssessmentProgress['status'] : 'waiting',
    updated_at: typeof item.updated_at === 'string' ? item.updated_at : null,
    events: asArray<AssessmentProgressEvent>(item.events),
  }
}

/** 查询诊断进度及阶段事件（前端实时面板轮询用） */
export async function getAssessmentProgress(id: string): Promise<AssessmentProgress> {
  const response: unknown = await request.get(`/assessment/${id}/progress`)
  return normalizeAssessmentProgress(response)
}
...
export async function streamAssessmentProgress(
  ...
  const consume = (block: string) => {
    ...
    if (!dataLine) return
    try {
      onProgress(normalizeAssessmentProgress(JSON.parse(dataLine.slice(5).trim())))
    } catch {
...
/** 当前用户的评估历史 */
export async function getAssessmentList(): Promise<AssessmentListItem[]> {
  const response: unknown = await request.get('/assessment/list')
  return asArray<AssessmentListItem>(response)
}

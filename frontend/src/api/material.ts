...
export async function getMaterialList(params?: { job_id?: string; assessment_id?: string }): Promise<UserMaterial[]> {
  const response: unknown = await request.get('/material/list', { params })
  return asArray<UserMaterial>(response)
}

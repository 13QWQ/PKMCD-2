# 职学导航：目标能力诊断与个性化学习资源生成系统

> 本项目用于 2026 科大讯飞 AI 开发者大赛「岗位胜任力多轮问询与评估报告生成」赛道参赛。

面向计算机类职业学习者的多 Agent 能力诊断、资料审查与个性化学习资源生成平台。

系统服务于已经明确目标岗位、但需要判断自身能力是否达标的学习者。用户提交学习描述、简历、项目材料和课程记录后，系统提取能力证据，对照岗位能力模型识别差距，再基于领域知识库生成学习路径与学习资源，并对生成内容进行来源校验。
...
本仓库 `main` 分支是项目实现与技术文档的唯一事实来源。方案书、技术文档、部署文档和答辩材料引用系统能力时，应同时注明所依据的 Git 提交号；压缩包、聊天记录和旧版 README 只能作为交接材料，不能覆盖当前代码事实。

- 系统固定为 **7 个业务 Agent**，不是 6 个或 8 个。资料审查多轮对话是正式流程前的准入策略，防幻觉校验是资源发布护栏，均不另计 Agent。
- “7 个 Agent 阶段接口”指后端内部的 7 个 Python `run()` 调用入口，不等于必须提供 7 个 HTTP 接口。前端通过统一业务 API 发起任务、读取进度和获取结果。
- 置信度、自动证据校准得分和真实准确率是三个不同指标。只有使用客观题、实操结果或专家标注计算的结果，才能作为比赛真实准确率。
- 业务数据库保存用户、资料、诊断、资源、路径和日志；Qdrant 保存可检索的领域知识向量及原始片段元数据。二者不能混称为“数据库”或“知识库”。
- 当前后端基线是 Python + FastAPI + SQLAlchemy + SQLite；Qdrant + BGE-M3 负责向量检索；DeepSeek 或其他 OpenAI 兼容 API 负责需要生成和语义判断的环节。未配置外部模型时进入规则降级模式，不代表外部模型已经接入。
...
| Agent 编排 | Python 串行运行时、阶段化 Prompt、结构化 JSON 输出与精确上下文账本 |
...
七个 Agent 的代码入口、提示词契约和阶段接口如下。`backend/prompts/` 用于记录可审计的阶段目标与输出约束；实际模型请求以运行时代码中的系统提示词、结构化输入和输出校验共同组成，不能把提示词目录误写成唯一执行入口。
...
诊断执行期间，后端维护有界进度事件序列。能力诊断页优先通过 SSE 接收实时事件，连接不可用时回退到约 2.5 秒一次的轮询；资料审查页在提交阶段采用约 0.9 秒一次的轮询。不要在技术文档中把整个系统统一写成固定 0.9 秒轮询。

```http
GET /api/assessment/{assessment_id}/progress
GET /api/assessment/{assessment_id}/progress/stream
```
...
当前仓库报告中的知识库统计必须分成三个口径，不得只写一个“切片总数”：

| 口径 | 数量 | 含义 |
|---|---:|---|
| 原始候选切片 | 23,504 | 进入清洗、映射和审核流程的全部候选片段 |
| 已入库切片 | 12,026 | 已写入 `career_knowledge_v1` 的可检索片段（另有 12,583 条完成岗位要求映射） |
| 待映射未入库切片 | 10,921 | 因岗位要求映射尚未完成而跳过，不能作为当前可检索知识量 |

当前集合使用 `BAAI/bge-m3`、1024 维向量。`knowledge/reports/retrieval_eval_report.json` 记录的 40 条独立改写查询评测结果为：Recall@10 85.0%、MRR@10 0.6690、跨岗位误召回率 0.0%、`source_chunk_id` 返回率 100.0%。这些是检索评测指标，不是能力诊断准确率。
...
| 认证 | `PUT /api/auth/active-assessment` | 切换当前诊断，使能力诊断与资料库同步 |
| 岗位 | `GET /api/jobs/list` | 岗位能力模型列表 |
| 资料 | `GET /api/material/list` | 当前用户资料列表 |
| 资料 | `POST /api/material/upload` | 上传并解析资料 |
| 资料 | `POST /api/material/text` | 保存文本证据 |
| 资料 | `GET /api/material/{id}/download` | 下载资料 |
| 资料 | `DELETE /api/material/{id}` | 删除资料 |
| 评估 | `GET /api/assessment/list` | 诊断历史列表 |
| 评估 | `POST /api/assessment/create` | 创建评估 |
| 会话 | `POST /api/session/create` | 创建默认两轮的资料审查会话 |
| 会话 | `GET /api/session/{id}` | 获取资料审查会话状态 |
...
| 评估 | `GET /api/assessment/{id}` | 获取诊断记录 |
| 评估 | `GET /api/assessment/{id}/diagnosis` | 获取能力诊断结果 |
| 评估 | `GET /api/assessment/{id}/progress` | 实时进度事件 |
| 评估 | `GET /api/assessment/{id}/progress/stream` | SSE 实时进度流 |
| 评估 | `GET /api/assessment/{id}/agents` | Agent 轨迹与检索来源 |
| 校准 | `POST /api/assessment/{id}/calibrate` | 提交真实标注并校准 |
| 校准 | `POST /api/assessment/{id}/auto-calibrate` | 自动复核已有证据，不等同真实准确率 |
| 评估 | `POST /api/assessment/{id}/repair-learning-package` | 为历史诊断原子化重建资源与路径 |
| 资源 | `GET /api/resource/list` | 用户资源列表 |
| 资源 | `GET /api/resource/search` | 搜索资源 |
| 资源 | `GET /api/resource/{id}` | 获取含正文与来源的资源详情 |
| 收藏 | `GET /api/resource/bookmarks` | 收藏列表 |
| 收藏 | `POST /api/resource/{id}/bookmark` | 收藏资源 |
| 收藏 | `DELETE /api/resource/{id}/bookmark` | 取消收藏 |
| 路径 | `GET /api/path/{user_id}` | 获取当前诊断对应的学习路径 |
